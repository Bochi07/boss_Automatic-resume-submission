...
(function () {
    ...
    class WebBroadcast {
                ...
                const trySend = (attempt) => {
                    try {
                        ...
                        if (attempt < this.retry) {
                            setTimeout(() => trySend(attempt + 1), this.retryInterval);
                        } else {
        ...
        destroy() {
            ...
            // 清理所有 pending 的超时定时器
            Object.values(this.pendingResponses).forEach(p => clearTimeout(p.timer));
            Object.values(this.pendingReceives).forEach(p => clearTimeout(p.timer));
            this.pendingResponses = {};
            this.pendingReceives = {};
            this.evts = {};
        }
    ...
    class Zhipin {
        ...
        async __chat() {
            ...
            const sayHi = async () => {
                ...
                let count = 0;
                const heartLoop = () => {
                    if (!heartAlive || this.broadcast._destroyed) return;
            ...
            const getChatInfo = async () => {
                ...
                const scroll2Top = async (maxRetries = 10) => {
