try:
    ...
    chat = None
    Message = None
from prompts import INTRODUCE, TAGS, CHARACTER, CHAT, INTEREST, NEEDRESUME, NEEDWORKS
...
def __streamChat(sys_prompt: str, prompt: str, options: dict = None, model: str = None) -> str:
    """自定义的流式回复"""
...
def __calcInterestValue(msgs: list):
    """计算兴趣值"""
    __ensure_ollama_available()
    msgs = [Message(role='system', content=INTEREST)] + msgs
...
def replyMsg(msgs: list, resume: str, character: str):
    ...
    # 获取回复（使用新列表，不修改入参）
    content = ''
    chat_msgs = [Message(role='system', content=CHAT.format(
        resume=resume,
        character=character
    ))] + msgs
    for i in chat(Config.chat_model, messages=chat_msgs, stream=True, options={
        "temperature": 0.4,
        "num_ctx": 10240,
    }):
        word = i.message.content
...
def isNeedResume(msgs: list):
    """判断是否需要简历"""
    __ensure_ollama_available()
    _msgs = [Message(role='system', content=NEEDRESUME)] + msgs
    return json.loads(chat(Config.chat_model, _msgs, format=NeedResume.model_json_schema(), options={
        "temperature": 0.2,
        "num_ctx": 10240,
...
def isNeedWorks(msgs: list):
    """判断是否需要作品集"""
    __ensure_ollama_available()
    _msgs = [Message(role='system', content=NEEDWORKS)] + msgs
    return json.loads(chat(Config.chat_model, _msgs, format=NeedWorks.model_json_schema(), options={
        "temperature": 0.2,
        "num_ctx": 10240,
    }).message.content)['need']
